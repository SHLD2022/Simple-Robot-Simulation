function plt = PlotFrame(T,varargin)
p = inputParser;
...
plt(1) = plot3(xl(1,:),xl(2,:),xl(3,:),style,'Color',r(colorind,:),'LineWidth',linewidth);hold on
plt(2) = plot3(yl(1,:),yl(2,:),yl(3,:),style,'Color',g(colorind,:),'LineWidth',linewidth);hold on
plt(3) = plot3(zl(1,:),zl(2,:),zl(3,:),style,'Color',b(colorind,:),'LineWidth',linewidth);hold on

if arrowsize~=0
plt(4:9) = arrow3(arrowsize*scale,T,scale,[r(colorind,:);g(colorind,:);b(colorind,:)]);
end

if strcmpi(texton,'on')
    plt(10) = text(x(1,2),x(2,2),x(3,2),['X',frameind],'FontWeight','bold','Color',r(colorind,:));hold on
    plt(11) = text(y(1,2),y(2,2),y(3,2),['Y',frameind],'FontWeight','bold','Color',g(colorind,:));hold on
    plt(12) = text(z(1,2),z(2,2),z(3,2),['Z',frameind],'FontWeight','bold','Color',b(colorind,:));hold on
end
end

function plt = arrow3(h,T,scale,color3)
% Draw Arrow
r=[h*tand(15),0];
[x,y,z]=cylinder(r,20);
z = z*h;
...
plt = gobjects(6,1);
plt(1) = mesh([x_1(1,:);x_2(1,:)],[x_1(2,:);x_2(2,:)],[x_1(3,:);x_2(3,:)],...
    'FaceColor',color3(1,:),'EdgeColor','none'); hold on
plt(2) = fill3(x_1(1,:),x_1(2,:),x_1(3,:),color3(1,:),'EdgeColor','none'); hold on
plt(3) = mesh([y_1(1,:);y_2(1,:)],[y_1(2,:);y_2(2,:)],[y_1(3,:);y_2(3,:)],...
    'FaceColor',color3(2,:),'EdgeColor','none'); hold on
plt(4) = fill3(y_1(1,:),y_1(2,:),y_1(3,:),color3(2,:),'EdgeColor','none'); hold on
plt(5) = mesh([z_1(1,:);z_2(1,:)],[z_1(2,:);z_2(2,:)],[z_1(3,:);z_2(3,:)],...
    'FaceColor',color3(3,:),'EdgeColor','none'); hold on
plt(6) = fill3(z_1(1,:),z_1(2,:),z_1(3,:),color3(3,:),'EdgeColor','none'); hold on
